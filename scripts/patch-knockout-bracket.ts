/**
 * Update r8–final placeholder team names in Firestore (preserves scores).
 * Run: npm run db:patch-knockout-bracket
 */
...
async function main() {
  const knockout = MATCHES.filter((m) => m.stage !== "group" && m.stage !== "r16");
  ...
  await batch.commit();
  console.log(
    `Updated ${knockout.length} knockout matches (r8–final) with bracket placeholders.`,
  );
}
